...
static const char *DescriptionMsg = "Remove unused function declarations. \n";
...
typedef llvm::DenseMap<const clang::UsingDecl *, const clang::FunctionDecl *>
    UsingFunctionDeclsMap;

typedef llvm::SmallPtrSet<const clang::FunctionDecl *, 32> FunctionDeclsSet;
...
class UsingDeclVisitor : public RecursiveASTVisitor<UsingDeclVisitor> {
public:
  UsingDeclVisitor(const FunctionDecl *FD, RemoveUnusedFunction *Instance)
      : CurrentFD(FD), ConsumerInstance(Instance) {}
...
class SpecializationVisitor
    : public RecursiveASTVisitor<SpecializationVisitor> {
public:
...
bool SpecializationVisitor::VisitCXXOperatorCallExpr(CXXOperatorCallExpr *E) {
  // ConsumerInstance->handleOneCXXOperatorCallExpr(E);
  return true;
}

bool SpecializationVisitor::VisitCallExpr(CallExpr *E) {
...
class ReferencedFunctionDeclVisitor
    : public RecursiveASTVisitor<ReferencedFunctionDeclVisitor> {
public:
...
class ExtraReferenceVisitorWrapper
    : public RecursiveASTVisitor<ExtraReferenceVisitorWrapper> {
public:
...
bool RUFAnalysisVisitor::VisitFunctionDecl(FunctionDecl *FD) {
  ...
  if (TK == FunctionDecl::TK_MemberSpecialization) {
    ...
    if (Info->getNumTemplates() > 0) {
      const FunctionDecl *Member = Info->getTemplate(0)->getTemplatedDecl();
      ConsumerInstance->addOneMemberSpecialization(FD, Member);
    }
    return true;
  }
  ...
  if (FD->isReferenced() || FD->isMain() || FD->hasAttr<OpenCLKernelAttr>() ||
...
SourceLocation RemoveUnusedFunction::getExtensionLocStart(SourceLocation Loc) {
  ...
  TransAssert((ExtStr == "__extension__") && "Invalid ExtStr!");
  (void)ExtStr;
...
SourceLocation
RemoveUnusedFunction::getFunctionOuterLocStart(const FunctionDecl *FD) {
...
SourceLocation RemoveUnusedFunction::getFunctionLocEnd(SourceLocation LocStart,
...
void RemoveUnusedFunction::removeFunctionExplicitInstantiations(
  ...
  MemberSpecializationSet *ExplicitSpecs = FuncToExplicitSpecs[CanonicalFD];
  ...
  if (!S)
    return;
  for (MemberSpecializationSet::iterator I = S->begin(), E = S->end(); I != E;
       ++I) {
...
const FunctionDecl *RemoveUnusedFunction::getFunctionDeclFromSpecifier(
  ...
  case NestedNameSpecifier::Namespace:
    FD = lookupFunctionDeclShallow(Name, NNS->getAsNamespace());
    break;
  ...
  case NestedNameSpecifier::Global:
    FD = lookupFunctionDeclShallow(Name, Context->getTranslationUnitDecl());
    break;
  default:
    return NULL;
  }
...
void RemoveUnusedFunction::handleOneCXXDependentScopeMemberExpr(
    const FunctionDecl *CurrentFD, const CXXDependentScopeMemberExpr *E) {
...
void RemoveUnusedFunction::handleOneUnresolvedLookupExpr(
    const FunctionDecl *CurrentFD, const UnresolvedLookupExpr *E) {
...
void RemoveUnusedFunction::handleOneMemberExpr(const MemberExpr *ME) {
...
const FunctionDecl *
RemoveUnusedFunction::getSourceFunctionDecl(const FunctionDecl *TheFD) {
...
void RemoveUnusedFunction::addOneReferencedFunction(const FunctionDecl *FD) {
  ReferencedFDs.insert(FD->getCanonicalDecl());
}

const FunctionTemplateDecl *
RemoveUnusedFunction::getTopDescribedTemplate(const FunctionDecl *FD) {
...
void RemoveUnusedFunction::addOneMemberSpecialization(
  ...
  MemberSpecializationSet *S = MemberToSpecs[Member->getCanonicalDecl()];
  if (S == NULL) {
    S = new MemberSpecializationSet();
...
void RemoveUnusedFunction::createFuncToExplicitSpecs(const FunctionDecl *FD) {
  MemberSpecializationSet *S = FuncToExplicitSpecs[FD->getCanonicalDecl()];
  if (S == NULL) {
    S = new MemberSpecializationSet();
