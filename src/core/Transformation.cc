...
class TransNameQueryVisitor
    : public RecursiveASTVisitor<TransNameQueryVisitor> {

public:
  TransNameQueryVisitor(TransNameQueryWrap *Instance, const std::string &Prefix)
      : WrapInstance(Instance), NamePrefix(Prefix) {}
...
TransNameQueryWrap::TransNameQueryWrap(const std::string &Prefix)
    : NamePrefix(Prefix), MaxPostfix(0) {
  NameQueryVisitor = new TransNameQueryVisitor(this, Prefix);
}

TransNameQueryWrap::~TransNameQueryWrap(void) { delete NameQueryVisitor; }
...
const Expr *Transformation::ignoreSubscriptExprParenCasts(const Expr *E) {
...
const Expr *Transformation::getInitExprByIndex(IndexVector &Idxs,
  ...
  for (IndexVector::const_reverse_iterator I = Idxs.rbegin(), E = Idxs.rend();
       I != E; ++I) {
    Count++;
    unsigned int Idx;
...
const Expr *
Transformation::getArrayBaseExprAndIdxs(const ArraySubscriptExpr *ASE,
                                        IndexVector &Idxs) {
...
const Expr *
Transformation::getArraySubscriptElem(const ArraySubscriptExpr *ASE) {
  IndexVector ArrayDims;
  const Expr *BaseE = getArrayBaseExprAndIdxs(ASE, ArrayDims);
  return getInitExprFromBase(BaseE, ArrayDims);
}

const Expr *Transformation::getMemberExprBaseExprAndIdxs(const MemberExpr *ME,
                                                         IndexVector &Idxs) {
  const Expr *BaseE = NULL;
  while (ME) {
...
unsigned int
Transformation::getArrayDimensionAndTypes(const ArrayType *ArrayTy,
...
unsigned int
Transformation::getConstArraySize(const ConstantArrayType *CstArrayTy) {
  unsigned int Sz;
...
const Expr *
Transformation::getBaseExprAndIdxExprs(const ArraySubscriptExpr *ASE,
                                       ExprVector &IdxExprs) {
  const Expr *BaseE = NULL;
  while (ASE) {
    const Expr *IdxE = ASE->getIdx();
...
const Type *Transformation::getBaseType(const Type *T) {
  if (T->isPointerType() || T->isReferenceType())
    ...
    return getBaseType(
        T->castAsArrayTypeUnsafe()->getElementType().getTypePtr());
...
// It's slow...
const FunctionDecl *
Transformation::lookupFunctionDeclInGlobal(DeclarationName &DName,
...
const FunctionDecl *
Transformation::lookupFunctionDeclFromBases(DeclarationName &DName,
...
const FunctionDecl *
Transformation::lookupFunctionDeclFromCtx(DeclarationName &DName,
                                          const DeclContext *Ctx,
                                          DeclContextSet &VisitedCtxs) {
  ...
  for (auto I = Result.begin(), E = Result.end(); I != E; ++I) {
    ...
    if (const UnresolvedUsingValueDecl *UUD =
      ...
      if (!Ctx)
        continue;
      if (const FunctionDecl *FD = lookupFunctionDecl(DName, Ctx, VisitedCtxs))
        return FD;
    }
  }
  return NULL;
...
const FunctionDecl *
Transformation::lookupFunctionDecl(DeclarationName &DName,
                                   const DeclContext *Ctx,
                                   DeclContextSet &VisitedCtxs) {
  if (dyn_cast<LinkageSpecDecl>(Ctx))
    return NULL;
...
const DeclContext *
Transformation::getDeclContextFromSpecifier(const NestedNameSpecifier *NNS) {
...
unsigned
Transformation::getNumCtorWrittenInitializers(const CXXConstructorDecl &Ctor) {
  unsigned Num = 0;
...
bool Transformation::isBeforeColonColon(TypeLoc &Loc) {
  ...
  SourceLocation ColonColonLoc = Lexer::findLocationAfterToken(
      EndLoc, tok::coloncolon, *SrcManager, Context->getLangOpts(),
...
bool Transformation::getTemplateTypeParmString(
    const TemplateTypeParmType *ParmTy, const TemplateArgument *Args,
    unsigned NumArgs, std::string &Str) {
  unsigned Idx = ParmTy->getIndex();
...
bool Transformation::getTypedefString(const StringRef &Name,
  ...
  for (DeclContext::decl_iterator I = CXXRD->decls_begin(),
    ...
    if (TC == Type::DependentName) {
      if (replaceDependentNameString(UnderlyingTy, Args, NumArgs, Str,
                                     Typename))
        return true;
...
bool Transformation::getDependentNameTypeString(const DependentNameType *DNT,
                                                std::string &Str,
                                                bool &Typename) {
  const IdentifierInfo *IdInfo = DNT->getIdentifier();
  if (!IdInfo)
  ...
  return getTypedefString(IdInfo->getName(), BaseDef, Args, NumArgs, Str,
                          Typename);
}

bool Transformation::getTypeString(const QualType &QT, std::string &Str,
                                   bool &Typename) {
