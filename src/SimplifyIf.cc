...
class SimplifyIfCollectionVisitor
    : public RecursiveASTVisitor<SimplifyIfCollectionVisitor> {
public:
...
static RegisterTransformation<SimplifyIf> Trans("simplify-if", DescriptionMsg);

class SimplifyIfStatementVisitor
    : public CommonStatementVisitor<SimplifyIfStatementVisitor> {
public:
...
bool SimplifyIfStatementVisitor::VisitIfStmt(IfStmt *IS) {
  ...
  llvm::outs() << IS->getIfLoc().isInvalid() << "\n";
