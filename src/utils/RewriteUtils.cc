...
unsigned RewriteUtils::getOffsetBetweenLocations(SourceLocation StartLoc,
                                                 ...
                                                 SourceManager *SrcManager) {
  FileID FID;
  unsigned StartOffset = getLocationOffsetAndFileID(StartLoc, FID, SrcManager);
  unsigned EndOffset = getLocationOffsetAndFileID(EndLoc, FID, SrcManager);
...
SourceLocation RewriteUtils::getLocationUntil(SourceLocation Loc, char Symbol) {
...
SourceLocation RewriteUtils::getLocationAfter(SourceLocation Loc, char Symbol) {
  const char *Buf = SrcManager->getCharacterData(Loc);
  int Offset = getOffsetUntil(Buf, Symbol);
  Offset++;
  return Loc.getLocWithOffset(Offset);
...
SourceLocation
RewriteUtils::getParamSubstringLocation(SourceLocation StartLoc, size_t Size,
...
bool RewriteUtils::removeParamFromFuncDecl(const ParmVarDecl *PV,
  ...
  if ((ParamPos == 0) && (NumParams == 1)) {
    return !(TheRewriter->ReplaceText(StartLoc, RangeSize, "void"));
  }
...
const Expr *RewriteUtils::getArgWrapper(const Expr *E, int ParamPos) {
...
bool RewriteUtils::removeArgFromExpr(const Expr *E, int ParamPos) {
...
bool RewriteUtils::removeArgFromCallExpr(const CallExpr *CallE, int ParamPos) {
  return removeArgFromExpr(CallE, ParamPos);
}

void RewriteUtils::skipRangeByType(const std::string &BufStr, const Type *Ty,
                                   int &Offset) {
  Offset = 0;
...
bool RewriteUtils::removeVarFromDeclStmt(DeclStmt *DS, const VarDecl *VD,
                                         Decl *PrevDecl, bool IsFirstDecl,
                                         bool *StmtRemoved) {
  ...
  if (IsFirstDecl) {
    ...
    return !(TheRewriter->RemoveText(SourceRange(NewStartLoc, NewEndLoc)));
  }
...
bool RewriteUtils::getExprString(const Expr *E, std::string &ES) {
...
bool RewriteUtils::getStmtString(const Stmt *S, std::string &Str) {
...
bool RewriteUtils::replaceExpr(const Expr *E, const std::string &ES) {
  SourceRange ExprRange = E->getSourceRange();
...
bool RewriteUtils::replaceExprNotInclude(const Expr *E, const std::string &ES) {
  SourceRange ExprRange = E->getSourceRange();
  SourceLocation StartLoc = ExprRange.getBegin();
  if (StartLoc.isMacroID()) {
    StartLoc = SrcManager->getFileLoc(StartLoc);
...
const char *RewriteUtils::getTmpVarNamePrefix(void) { return TmpVarNamePrefix; }

bool RewriteUtils::addNewAssignStmtBefore(Stmt *BeforeStmt,
                                          const std::string &VarName, Expr *RHS,
                                          bool NeedParen) {
  std::string IndentStr =
  ...
  if (NeedParen) {
    ...
    SourceLocation LocEnd = RewriteUtils::getEndLocationFromBegin(StmtRange);
  ...
  return !(TheRewriter->InsertText(StmtLocStart, AssignStmtStr,
                                   /*InsertAfter=*/false));
}

void RewriteUtils::indentAfterNewLine(StringRef Str, std::string &NewStr,
...
void RewriteUtils::addOpenParenBeforeStmt(Stmt *S,
                                          const std::string &IndentStr) {
  SourceRange StmtRange = S->getSourceRange();
  SourceLocation LocEnd = RewriteUtils::getEndLocationFromBegin(StmtRange);
  TransAssert(LocEnd.isValid() && "Invalid LocEnd!");
...
bool RewriteUtils::addStringBeforeStmtInternal(Stmt *S, const std::string &Str,
                                               const std::string &IndentStr,
                                               bool NeedParen) {
  std::string NewStr;
  ...
  return !(TheRewriter->InsertText(S->getLocStart(), IndentedStr,
                                   /*InsertAfter=*/false));
}

bool RewriteUtils::addStringBeforeStmt(Stmt *BeforeStmt, const std::string &Str,
                                       bool NeedParen) {
  std::string IndentStr =
      RewriteUtils::getStmtIndentString(BeforeStmt, SrcManager);
  ...
  return addStringBeforeStmtInternal(BeforeStmt, Str, IndentStr, NeedParen);
}
...
bool RewriteUtils::addStringBeforeStmtAndReplaceExpr(Stmt *BeforeStmt,
  ...
  return addStringBeforeStmtInternal(BeforeStmt, StmtStr, IndentStr, NeedParen);
}

bool RewriteUtils::addStringAfterStmt(Stmt *AfterStmt, const std::string &Str) {
  std::string IndentStr =
...
bool RewriteUtils::replaceNamedDeclName(const NamedDecl *ND,
  ...
  return !(TheRewriter->ReplaceText(NameLocStart, ND->getNameAsString().size(),
                                    NameStr));
}

bool RewriteUtils::replaceValueDecl(const ValueDecl *VD,
                                    const std::string &Str) {
...
bool RewriteUtils::replaceVarDeclName(VarDecl *VD, const std::string &NameStr) {
  SourceLocation NameLocStart = VD->getLocation();
  return !(TheRewriter->ReplaceText(NameLocStart, VD->getNameAsString().size(),
                                    NameStr));
}
...
bool RewriteUtils::replaceFunctionDeclName(const FunctionDecl *FD,
  ...
  return !TheRewriter->ReplaceText(NameInfo.getLoc(), FDNameLen, NameStr);
}

bool RewriteUtils::replaceCXXDestructorDeclName(
    const CXXDestructorDecl *DtorDecl, const std::string &Name) {
  ...
  return !TheRewriter->ReplaceText(StartLoc, NameLen, "~" + Name);
}

bool RewriteUtils::replaceRecordDeclName(const RecordDecl *RD,
                                         const std::string &NameStr) {
  SourceLocation LocStart = RD->getLocation();
  return !TheRewriter->ReplaceText(LocStart, RD->getNameAsString().length(),
                                   NameStr);
}
...
bool RewriteUtils::replaceVarTypeName(const VarDecl *VD,
  ...
  return !TheRewriter->ReplaceText(LocStart, TypeId->getLength(), NameStr);
}
...
bool RewriteUtils::getDeclStmtStrAndRemove(DeclStmt *DS, std::string &Str) {
  DeclGroupRef DGR = DS->getDeclGroup();
...
bool RewriteUtils::removeASymbolAfter(const Expr *E, char Symbol) {
  SourceRange ExprRange = E->getSourceRange();
...
bool RewriteUtils::removeVarDecl(const VarDecl *VD, DeclGroupRef DGR) {
  SourceRange VarRange = VD->getSourceRange();
...
bool RewriteUtils::replaceCXXMethodNameAfterQualifier(
    const NestedNameSpecifierLoc *QualLoc, const CXXMethodDecl *MD,
