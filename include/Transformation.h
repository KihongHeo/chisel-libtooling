...
public:
  Transformation(const char *TransName, const char *Desc)
      : Name(TransName), TransformationCounter(-1), ValidInstanceNum(0),
        QueryInstanceOnly(false), Context(NULL), SrcManager(NULL),
  ...
  void setToCounter(int Counter) { ToCounter = Counter; }

  bool isMultipleRewritesEnabled() { return MultipleRewrites; }
  ...
  bool transSuccess() { return (TransError == TransSuccess); }

  bool transMaxInstanceError() { return (TransError == TransMaxInstanceError); }

  bool transInternalError() { return (TransError == TransInternalError); }
  ...
  std::string &getDescription() { return DescriptionString; }

  void getTransErrorMsg(std::string &ErrorMsg);

  int getNumTransformationInstances() { return ValidInstanceNum; }

  virtual bool skipCounter() { return false; }
...
protected:
  ...
  const clang::Expr *ignoreSubscriptExprParenCasts(const clang::Expr *E);
  ...
  const clang::Expr *
  getBaseExprAndIdxExprs(const clang::ArraySubscriptExpr *ASE,
                         ExprVector &IdxExprs);
  ...
  lookupFunctionDeclFromCtx(clang::DeclarationName &DName,
                            const clang::DeclContext *Ctx,
                            DeclContextSet &VisitedCtxs);

  const clang::FunctionDecl *
  lookupFunctionDeclFromBases(clang::DeclarationName &DName,
                              const clang::CXXRecordDecl *CXXRD,
                              DeclContextSet &VisitedCtxs);

  const clang::FunctionDecl *
  lookupFunctionDeclInGlobal(clang::DeclarationName &DName,
                             const clang::DeclContext *Ctx);

  const clang::DeclContext *
  getDeclContextFromSpecifier(const clang::NestedNameSpecifier *NNS);
...
public:
  ...
  unsigned int getMaxNamePostfix() { return MaxPostfix; }
