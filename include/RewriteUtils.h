...
public:
  ...
  bool removeParamFromFuncDecl(const clang::ParmVarDecl *PV,
                               unsigned int NumParams, int ParamPos);

  bool removeArgFromCallExpr(const clang::CallExpr *CallE, int ParamPos);

  bool removeArgFromCXXConstructExpr(const clang::CXXConstructExpr *CE,
                                     int ParamPos);

  bool removeVarFromDeclStmt(clang::DeclStmt *DS, const clang::VarDecl *VD,
                             clang::Decl *PrevDecl, bool IsFirstDecl,
                             bool *StmtRemoved);

  bool getExprString(const clang::Expr *E, std::string &ES);

  bool getStmtString(const clang::Stmt *S, std::string &Str);

  bool replaceExpr(const clang::Expr *E, const std::string &ES);

  bool replaceExprNotInclude(const clang::Expr *E, const std::string &ES);

  bool addLocalVarToFunc(const std::string &VarStr, clang::FunctionDecl *FD);

  std::string getStmtIndentString(clang::Stmt *S,
                                  clang::SourceManager *SrcManager);

  bool addNewAssignStmtBefore(clang::Stmt *BeforeStmt,
                              const std::string &VarName, clang::Expr *RHS,
                              bool NeedParen);

  bool addStringBeforeStmt(clang::Stmt *BeforeStmt, const std::string &Str,
                           bool NeedParen);
  ...
  bool replaceVarDeclName(clang::VarDecl *VD, const std::string &NameStr);
  ...
  bool replaceVarTypeName(const clang::VarDecl *VD, const std::string &NameStr);

  const char *getTmpVarNamePrefix(void);

  void getStringBetweenLocs(std::string &Str, clang::SourceLocation LocStart,
  ...
  bool getDeclGroupStrAndRemove(clang::DeclGroupRef DGR, std::string &Str);
  ...
  bool getDeclStmtStrAndRemove(clang::DeclStmt *DS, std::string &Str);
  ...
  bool removeVarDecl(const clang::VarDecl *VD, clang::DeclGroupRef DGR);
...
private:
  ...
  RewriteUtils(void) : TheRewriter(NULL), SrcManager(NULL) {}
  ...
  void indentAfterNewLine(llvm::StringRef Str, std::string &NewStr,
  ...
  clang::SourceLocation
  skipPossibleTypeRange(const clang::Type *Ty, clang::SourceLocation OrigEndLoc,
                        clang::SourceLocation VarStartLoc);

  void skipRangeByType(const std::string &BufStr, const clang::Type *Ty,
                       int &Offset);
