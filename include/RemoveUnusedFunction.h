...
public:
  RemoveUnusedFunction(const char *TransName, const char *Desc)
      : Transformation(TransName, Desc, /*MultipleRewrites*/ true),
        AnalysisVisitor(NULL), VisitorWrapper(NULL), TheFunctionDecl(NULL) {}
  ...
  void handleOneUnresolvedLookupExpr(const clang::FunctionDecl *CurrentFD,
...
private:
  ...
  typedef llvm::DenseMap<const clang::FunctionDecl *, MemberSpecializationSet *>
      MemberToSpecializationMap;

  typedef llvm::SmallPtrSet<const clang::UsingDecl *, 32> UsingDeclsSet;

  typedef std::map<std::string, std::string> InlinedSystemFunctionsMap;
  ...
  lookupFunctionDeclShallow(const clang::DeclarationName &DName,
                            const clang::DeclContext *Ctx);

  const clang::FunctionDecl *
  getFunctionDeclFromSpecifier(const clang::DeclarationName &Name,
  ...
  getTopDescribedTemplate(const clang::FunctionDecl *FD);

  const clang::FunctionDecl *
  getSourceFunctionDecl(const clang::FunctionDecl *TheFD);
